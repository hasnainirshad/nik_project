...
def generate_models(create_graph: CreateGraph,
    ...
    while True:
        message = yield model, stats
        stats.clear()

        if message[0] == "initial_proxy":
            labeled = message[1]
        elif message[0] == "proxy_model_train":
            pruned_model = message[1]
            labeled = message[2]

        elif message[0] == "fusion_and_train":
            pruned_model = message[1]
            labeled = message[2]
            prune_bool = True
            kd_bool = True

        elif message[0] == "fused_to_pruned":
            fused_model = message[1]
            labeled = message[2]
        ...
        if message[0] == "initial_proxy":
            model, optimizer = create_graph(model=None, device=device, run_dir=round_dir)
        elif message[0] == "proxy_model_train": #train the model as it is without pruning
            model, optimizer = create_graph(model=pruned_model, device=device, run_dir=round_dir)
        elif message[0] == "fusion_and_train": # use this to fuse and then train
            model, optimizer = create_graph(model=pruned_model, device=device, run_dir=round_dir)
        elif message[0] == "fused_to_pruned": #give the fused model, prune it and train
            model, optimizer = create_graph(model=None, fused_model=fused_model, device=device, run_dir=round_dir)
        ...
        if message[0] == "fusion_and_train":
            model, accuracies, times = run_training(
                ...
                fp16=fp16,
                run_dir=round_dir,
                checkpoint=checkpoint, pruned_model= pruned_model, prune_bool=prune_bool, kd_bool=kd_bool)
