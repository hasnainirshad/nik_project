...
def run_training(model: nn.Module, optimizer: Optimizer, criterion: nn.Module,
                 ...
                 run_dir: Optional[str] = None,
                 checkpoint: str = 'last',
                 prune_bool: bool = False, 
                 kd_bool: bool = False,
                 pruned_model: nn.Module = None
    ...
    for nepochs, learning_rate in zip(epochs, learning_rates):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            # #set the learning rate according to cosine annealing
            # for group in optimizer.param_groups:
            #     if epoch <= 10:
            #         group['lr'] = 0.5 * 0.1 * (1 + np.pi * epoch / nepochs) * (epoch - 1) / 10 + 0.01 * (11 - epoch) / 10
            #     else:
            #         group['lr'] = 0.5 * 0.1 * (1 + np.cos(np.pi * epoch / nepochs))
                
            
            # print(f'Epoch {epoch} learning rate set to {group["lr"]}')
            ...
            global_step, train_accs, epoch_train_time = run_epoch(
                ...
                batch_callback=batch_callback, fp16=fp16, prune_bool=prune_bool, kd_bool=kd_bool, pruned_model = pruned_model)
        ...
        #     wandb.log({
        #         'epoch': epoch, 
        #         'train_acc': train_accs.val,
        #         'val_acc': test_accs.val, 
        #     })
        # wandb.log({
        #         'nepoch': nepochs, 
        #     })
...
def run_epoch(epoch: int, global_step: int, model: nn.Module,
              ...
              batch_callback: Optional[Callable] = None,
              fp16: bool = False, prune_bool: bool = False, kd_bool: bool = False,  
              pruned_model: nn.Module = None
    ...
    #initializing parameters for KD
    temperature = 3.0
    alpha = 0.35
            ...
            for batch_index, (indices, inputs, targets) in enumerate(wrapped_loader):  # noqa: E501
                ...
                if train:
                    ...
                    if kd_bool:
                        ...
                        # Forward pass with smaller network
                        smaller_outputs = pruned_model(inputs)
                        smaller_probs = nn.functional.softmax(smaller_outputs / temperature, dim=1)

                        # Forward pass with larger network
                        larger_probs = nn.functional.softmax(outputs / temperature, dim=1)

                        # Compute the knowledge distillation loss
                        distillation_loss = nn.functional.kl_div(larger_probs.log(), smaller_probs.detach(), reduction='batchmean')

                        # Combine the distillation loss and classification loss
                    ...
                    if prune_bool:
                        ...
                        if fp16:
                            ...
                            pruned_state_dict = pruned_model.module.state_dict()
                            ...
                            for name, param in model.module.named_parameters():
                                # name, param = val1
                                if 'conv' in name:
                                        if state_dict[name].size(0) == 2 * pruned_state_dict[name].size(0) and state_dict[name].size(1)  == 2 * pruned_state_dict[name].size(1):
                                            if epoch in range(500, 600):
                                                param.grad[::2, ::2, :, :] = param.grad[::2, ::2, :, :] / 10
                                            elif epoch == 1:
                                                param.grad[::2, ::2, :, :] = torch.zeros_like(param.grad[::2, ::2, :, :])
                                        
                                        else:
                                            if epoch in range(500, 600):
                                                param.grad[:pruned_state_dict[name].size(0), :state_dict[name].size(1), :, :] = param.grad[:pruned_state_dict[name].size(0), :state_dict[name].size(1), :, :] / 10
                                            elif epoch == 1:
                                                param.grad[:pruned_state_dict[name].size(0), :state_dict[name].size(1), :, :] = torch.zeros_like(param.grad[:pruned_state_dict[name].size(0), :state_dict[name].size(1), :, :])
